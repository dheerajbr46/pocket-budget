...
export function Header({ title, forceMobile, onToggleView }) {
  return (
    <header className="flex items-center justify-between px-5 pb-4 pt-5 lg:border-b lg:border-slate-200 lg:bg-white lg:px-8 lg:py-4">
      <div>
      ...
      <div className="flex items-center gap-1.5">
        ...
        >
          {forceMobile ? <Monitor size={16} /> : <Smartphone size={16} />}
        </Button>
        <Button
          className="grid h-9 w-9 place-items-center rounded-xl bg-slate-100 text-slate-500 transition-colors duration-150 hover:bg-slate-200"
          aria-label="Search"
